...
export async function POST(request) {
  try {
    ...
    // Handle both old (messages array) and new (message + history) formats
    let message, history = [];
    ...
    const contextPrompt = history.length > 0
      ? 'Previous conversation context:\n' +
        history.map((msg) => `${msg.role === 'user' ? 'User' : 'Assistant'}: ${msg.content || msg.text || ''}`).join('\n') +
        '\nCurrent question: '
      : '';

    const fullPrompt = `${contextPrompt}${message}

You are a helpful pension and retirement planning assistant. Provide clear, accurate, and helpful responses about pension schemes, retirement planning, financial calculations, and related topics. Keep responses conversational but informative.`;
