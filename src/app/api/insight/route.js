export async function POST(req) {
  try {
    ...
    if (!process.env.GOOGLE_API_KEY) {
      ...
      return new Response(
        JSON.stringify({ error: 'Google API key is not configured. Please check your environment variables.' }),
    ...
    if (!data || !query) {
      return new Response(
        JSON.stringify({ error: 'Missing required parameters: data or query' }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      );
    }

    // System prompt to guide Gemini AI
    const systemPrompt = `
You are a financial advisor specializing in pension planning. Explain pension-related data and concepts in simple, easy-to-understand terms.
...
Please provide a clear, simple explanation and any helpful insights.`;
    ...
    if (result.candidates?.[0]?.content?.[0]?.parts?.[0]?.text) {
      const insight = result.candidates[0].content.parts[0].text;
      return new Response(JSON.stringify({ insight }), { status: 200, headers: { 'Content-Type': 'application/json' } });
    } else {
    ...
    if (error.message.includes('API key')) errorMessage = 'Invalid or missing Google API key';
    if (error.message.includes('quota')) errorMessage = 'API quota exceeded. Check your Google AI Studio account';
    if (error.message.includes('model')) errorMessage = 'Model not available. Check your API access';
    if (error.message.includes('Google API error')) errorMessage = 'Error communicating with Google AI service';
